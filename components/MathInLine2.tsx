...
export const MathInLine2: FC = () => {
    ...
    const handleTextareaChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        const pos = getPos(); // Obtener la posición del nodo
        const tr = state.tr;

        // Crear un nodo nuevo con los atributos actualizados
        const newNode = node.type.create({ ...node.attrs, value: newFormula });

        // Reemplazar el nodo en la posición correspondiente
        tr.replaceWith(pos, pos + node.nodeSize, newNode);
        dispatch(tr);

        // Actualizar la vista previa
        renderPreview(newFormula);
        renderInlineBlock(newFormula);
    };
    const addFormula = (newFormula: string) => {
        ...
        // Aquí también actualizar el nodo inline
